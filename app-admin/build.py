  ...
  <section id="view-settings" style="display:none">
    <div style="max-width:480px;display:flex;flex-direction:column;gap:24px;padding:8px 0">
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:20px 24px;display:flex;flex-direction:column;gap:12px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;gap:16px">
          <div>
            <div style="font-size:15px;font-weight:600">Recompute Best Scores</div>
            <div style="font-size:12px;color:var(--muted);margin-top:3px">The leaderboard reads each profile's best score. Games keep it current; run this after importing or editing game data by hand.</div>
            <div id="recompute-status" style="font-size:12px;color:var(--muted);margin-top:6px;min-height:16px"></div>
          </div>
          <button class="sort-btn" id="btn-recompute" onclick="recomputeBestScores()" style="white-space:nowrap">Recompute</button>
        </div>
      </div>
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:20px 24px;display:flex;flex-direction:column;gap:12px">
...
async function deletePlayerData(userId, deleteProfile) {{
  ...
  if (deleteProfile) {{
    ...
    // No games left, so they also drop off the leaderboard
    await F.updateDoc(profileRef, {{ game_count: 0, ...clearBest(F) }});
  }}
}}
